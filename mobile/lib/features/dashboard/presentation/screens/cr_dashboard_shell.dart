...
class CrDashboardShell extends StatelessWidget {
  final AuthController authController;
  final ValueNotifier<ThemeMode> themeModeNotifier;

  const CrDashboardShell({
    super.key,
  ...
  Future<void> _handleLogout(BuildContext context) async {
    final confirmed = await AppConfirmationDialog.show(
      context: context,
      title: 'Sign Out?',
      message: 'Are you sure you want to end your CR session and sign out?',
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const RoleBadgeChip(role: UserRole.cr),
                  const SizedBox(height: 12),
                  Text(
                    'Welcome back, ${user?.fullName ?? "CR"}!',
                    style: TextStyle(
                      fontSize: 22,
                      fontWeight: FontWeight.w800,
                      color: isDark ? Colors.white : const Color(0xFFC2410C),
                    ),
                  ),
                  const SizedBox(height: 6),
                  Text(
                    'Class Representative • Year ${user?.year ?? 3}, Sem ${user?.semester ?? 1}',
                    style: TextStyle(
                      fontSize: 14,
